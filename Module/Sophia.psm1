...
function AutoLogonWin
				...
				if (-not (Test-Path -Path Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\System))
				{
					New-Item -Path Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\System -ItemType Directory -Force
				}
				New-ItemProperty -Path Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\System -Name "AllowClipboardHistory" -PropertyType String -Value "1" -Force
    # Set-ItemProperty -Path 'registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\System' -Name AllowClipboardHistory -Value "0"
...
# Ne pas Toucher à l'ordre		
	...
	Remove-Item -Path "$env:USERPROFILE\Desktop\NonoOS\get-pip.py"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Flameshot.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\GitHub Desktop.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Google Assistant.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Obsidian.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Telegram.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\WhatsApp.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Bitwarden.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Discord.lnk"
	Remove-Item -Path "$env:USERPROFILE\Desktop\Tor Browser" -Recurse -Force
